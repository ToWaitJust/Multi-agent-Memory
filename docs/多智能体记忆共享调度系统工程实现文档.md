...
| M3 系统集成完成 | 2027.01.31 | 端到端 + 三层监控 + Studio 可视化（性能监测 + 可用客户端 GUI，研究者自用，不产品化） | ⏳         |
...
| full-ours      | `retriever.bm25`                          |   ✓   |   ✓  |   ✓  | 完整方案        |

**归因性保证**：baseline-naive 与 baseline-reme 锚定"调度层本身能带来多少提升"和"调度在强基座上的增益"两个维度；`+注意力`/`+权重`/`full-ours` 三个中性化基座组逐环叠加（同用 `retriever.bm25` 基座），每环增量可单独测量。
