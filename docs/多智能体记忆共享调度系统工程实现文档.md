...
├── dashboard.html                 # 控制台前端（7 阶段调度流 + 双池 PCA + 副线书签 + SSE 流式回答）
...
│   ├── agentscope_runtime.py      # 【2026-08-27】真实 AgentScope 2.0.6 主副线运行时（§4.8）
│   │                              #   Agent + Toolkit(Read/Write/Edit/Glob/Grep/Bash) + BYPASS 权限
│   ├── cli.py                     # CLI：run_one / run_ablation
│   ├── monitor/
...
│       ├── register_all.py        # 插件自动注册入口
│       ├── algorithms/            # 算法层插件（完整+桩），含 retrieval/ 子包
│       ├── infra/                 # ModelAdapter / EmbeddingBackend / AgentFactory
│       └── baselines/             # 基准/桩实现
├── analysis/                      # 实验与统计脚本
│   ├── gen_pipeline_trace.py      # 带埋点管线运行器 → data/pipeline_trace.json（全流程中间值）
│   ├── gen_ablation_dataset.py    # 生成消融数据集（data/ablation_dataset.jsonl）
│   ├── run_ablation.py            # 跑 5 组消融实验
│   ├── compute_metrics.py         # 6 项指标计算
...
│       ├── +weight.yaml
│       └── full_ours.yaml
├── data/                          # 运行期实验产物（不入库；与 logs/ 严格区分）
│   ├── reme/<user>/main/          # 主线常驻完整池 workspace（per-user）
│   ├── reme/<user>/sub*/          # 副线 workspace（含 shared_pool.json）
│   ├── metrics/                   # schedule.jsonl / reme_jobs.jsonl（调度埋点，非运行日志）
│   ├── runs/                      # run_manifest.csv + 单次实验目录
│   ├── embed_cache/               # embedding 落盘缓存（保证实验可复现）
│   └── pipeline_trace.json        # 全流程中间值 trace（gen_pipeline_trace.py 产出）
├── logs/                          # 【统一位置】运行日志（不入库）
│   └── dashboard.log              # 控制台服务日志（启动时重定向写入）
├── dev-tools-srtp/                # 本地工具链（模型权重 / pip 缓存，不入库）
├── tests/                         # 8 个单测模块（attention/core/middleware/plugins/...）
└── examples/
    ├── reme_demo.py               # 已有：ReMe 最小示例（保留，作为基线参考）
    └── sched_demo.py              # 新增：调度系统端到端演示
```

> **日志 vs 数据分区约定（2026-09-07 统一）**：`logs/` 只放**进程运行日志**（控制台 stdout/stderr 重定向、ReMe 应用日志）；`data/` 只放**实验数据产物**（埋点 JSONL、run_manifest、embedding 缓存、trace）。二者均在 `.gitignore` 中排除。
...
### 4.8.1 `srtp_memory/agentscope_runtime.py` — 真实 AgentScope 运行时与工具系统（2026-08-27）

> 本节记录**真实 AgentScope 2.0.6 接入**与**工具系统（Toolkit）**落地情况，供复现与排错。
> 与 §4.8 的区别：`agents.py` 是「Agent 工厂（抽象装配）」，本模块是「真实跑起来的运行时」。

**职责**：`SchedulingRuntime` 封装主副线执行，对外暴露 `main_answer(query, kept_texts)` 与
`sub_lines(query, kept_texts, main_answer, run_sub)`；控制台（`app_dashboard.py`）经它驱动智能体。

**后端选择**（启动时定一次，页面顶部标注 `agent_mode`）：

| 模式 | 触发条件 | 行为 |
| --- | --- | --- |
| `agentscope` | 装有 agentscope **且** 有 `DEEPSEEK_API_KEY` | 主线/副线用真实 AgentScope 2.0.6 `Agent` 实例驱动 |
| `headless` | 缺 agentscope 或缺 key | 降级为自研适配器（仍是真实 DashScope embedding + DeepSeek LLM），页面标注 |

**装配要点（2.0.6 实测 API，与旧文档/示例均不符）**：

| 项 | 正确写法 | 说明 |
| --- | --- | --- |
| Agent 类 | `from agentscope.agent import Agent` | 无 `agentscope.init`、无 `agentscope.agents`、无 `DialogAgent`/`ReActAgent` |
| 模型 | `DeepSeekChatModel(credential=DeepSeekCredential(api_key=key), model="deepseek-v4-flash")` | 与 `model.deepseek` 插件对齐 |
| 调用 | `asyncio.run(agent.reply(UserMsg(name="User", content=...)))` | `Agent` 实例**不可** `agent(msg)` 直接调用；`reply` 是 async |
| 文本提取 | 遍历 `Msg.content` 取 `TextBlock.text` | `Msg.content` 是 ContentBlock **列表**，不能当字符串用 |
| 工具集 | `Toolkit([Read(), Write(), Edit(), Glob(), Grep(), Bash(cwd=项目根)])` → `Agent(toolkit=...)` | 模块名是 `agentscope.tool`（无 `toolkit/service/tools`） |
| 权限 | `AgentState(permission_context=PermissionContext(mode=PermissionMode.BYPASS))` → `Agent(state=...)` | `AgentState` 在 **`agentscope.state`**，不在 `agentscope.agent` |

**已验证能力（真实 DeepSeek，2026-08-27 实测）**：

- 文件写入：`Write` 真实创建 `data/tool_demo/agent_demo.txt`（16s，Write→Read→中文总结）
- 命令执行：`Bash` 真实执行 `dir /b` 并正确列出文件（2.5s）
- 普通记忆问答：不触发工具，直接基于共享池记忆作答
- 工具自带 `.env`/`.ssh` 等 `dangerous_files` 保护，在 BYPASS 下仍生效

**已知坑点**：

- `PermissionMode.DONT_ASK` 会把所有请求转 **DENY** 导致 Agent 卡住（回复 "waiting for permission"），交互场景勿用；`BYPASS` 适合本地可信自用，`ACCEPT_EDITS`/`EXPLORE` 适合需审批/只读场景。
- Windows 下 `Bash` 走 `cmd /c`：`echo`/`dir`/`python --version` 正常，但 `python -c "带引号"` 输出为空（cmd 引号传递坑），应提示 Agent 少用复杂引号或改用脚本文件。
- ReAct 循环默认 `max_iters=20`，工具任务耗时可达十几秒至数十秒；`Toolkit.get_tool_schemas()` 是 async，需 await。
...
├── embed_cache/                   # embedding 落盘缓存（DashScope 1024 维，保证实验可复现）
├── pipeline_trace.json            # 全流程中间值 trace（8 阶段每步中间值）
└── runs/
    ├── run_manifest.csv           # 实验登记表（FR-10）
    └── run_20260818_a/            # 单次实验：traces.jsonl + schedule.jsonl + results.csv
```

**运行日志统一位置**（2026-09-07 起）：

```
logs/                              # 仅放进程运行日志，不入库
├── dashboard.log                  # 控制台服务日志（启动重定向写入）
└── YYYY-MM-DD_HH-MM-SS_<pid>.log  # ReMe 应用运行日志（第三方库产出）
```

> 启动控制台请重定向到 `logs/`：`python app_dashboard.py > logs/dashboard.log 2>&1 &`
> 实验数据（埋点 JSONL / run_manifest / embedding 缓存）放 `data/`，运行日志放 `logs/`，两者不混放。
...
| M3 系统集成完成 | 2027.01.31 | 端到端 + 三层监控 + 客户端控制台（性能监测 + 可用客户端 GUI，研究者自用，不产品化）。**实时交互控制台**：`app_dashboard.py` + `dashboard.html`（端口 8787，零依赖 stdlib http.server；SSE 推送 schedule/pools/sublines/answer_delta；7 阶段调度流 + 双池向量库 PCA 投影 + 副线书签）+ `srtp_memory/agentscope_runtime.py`（真实 AgentScope 2.0.6 主副线 Agent，装配 Toolkit 工具系统，可读写文件与执行命令）+ `analysis/gen_pipeline_trace.py`（带埋点管线运行器 → `data/pipeline_trace.json`，抓取入库/召回/四维打分/权重生成/排行榜/保留丢回/双池/LLM回答/耗时/token节省 每一步中间值，用于离线分析）。注：原离线流水线可视化 `app/`（端口 8890）已于 2026-09-07 移除，职责由 8787 实时控制台承担 | 🔄 控制台已落地 |
